...
//
//                   Whatsapp Bot Negro
//  𝙰𝙳𝙾𝙿𝚃𝙴𝙳 𝙵𝚁𝙾𝙼  𝚂𝙲𝚁𝙸𝙿𝚃 𝙾𝙵 𝙶𝙾𝙹𝙾-𝚂𝙰𝚃𝙾𝚁𝚄 𝓫𝔂 𝓷𝓮𝔁𝓾𝓼𝓝𝔀
//
//═══════════════════════════════════════════════════════//
...
async function startBot() {
    ...
    Bot.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./menu")(Bot, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
//welcome\\
        let nama = await Bot.getName(num)
...
Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/8bbe8a7de5c351dfcb077.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
                    Bot.sendMessage(anu.id, { image: Kon, contextInfo: { mentionedJid: [num] }, caption: `
⭐✑ Hi👋 @${num.split("@")[0]},
	...
	*/
    ...
    Bot.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("🐦Connection closed, reconnecting...."); Bot.end(`🐦Connection closed: ${reason}|${connection}`); startBot(); }
            ...
            else {Bot.end(`🐦Unknown DisconnectReason: ${reason}|${connection}`); startBot();}
        }
    ...
    Bot.react = (jid, emoji, id) => {
        ...
        Bot.sendMessage(jid, reactionMessage)
    }
